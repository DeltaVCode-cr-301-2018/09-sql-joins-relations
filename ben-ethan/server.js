  ...
  function queryTwo(onError) {
    ...
    client.query(SQL, values,
      function(err, result) {
        ...
        queryThree(result.rows[0].author_id, onError);
      }
    )
  }
...
app.put('/articles/:id', function(request, response, next) {
  let SQL = `UPDATE authors SET author=$2, author_url=$3 WHERE author_id=$1`;
  let values = [
  ...
  client.query(SQL, values)
    .then(() => {
      let SQL = `UPDATE articles SET author_id=$1, title=$2, category=$3, published_on=$4, body=$5 WHERE article_id=$6`;
      let values = [
        request.body.author_id,
        request.body.title,
        request.body.category,
...
// NOTE: Don't forget to `DROP TABLE articles`, since our schema has changed
function loadDB() {
  client.query(`
